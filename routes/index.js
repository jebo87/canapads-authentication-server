...
router.get('/consent', ensureAuthenticated, (req, res) => {
    ...
    fetch('https://***REMOVED***/oauth2/auth/requests/consent?' + querystring.stringify({ consent_challenge: challenge })
        , {
            agent
        }).then(function (response) {
            return response.json()
    ...
    fetch('https://***REMOVED***/oauth2/auth/requests/consent/accept?' + querystring.stringify({ consent_challenge: challenge }), {
        method: 'PUT',
        body: JSON.stringify(body),
        agent,
        headers: { 'Content-Type': 'application/json' }
